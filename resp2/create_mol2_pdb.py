...
def GenerateBox(pdbin, pdbout, box, nmol, tries):
    ...
    ferr.close()
    t0 = time.time()
    log.info("Running %s to create a solvent box..." % gmxcmd)
    stdout, stderr = p.communicate()
    log.info("Time elapsed: % .3f seconds" % (time.time() - t0))
    nmol_out = 0
...
def run_create_mol2_pdb(**kwargs):
    ...
    try:
        ...
        log.info("""
Warning Warning
You are using a predefinned pdb file, which was not created from this program.
This might be on purpose if the program has difficulties getting the right pdb file. 
If this was not your purpose please delete the following file:
{}S.pdb
Warning Warning
""".format(fullresname))
